        ...
        */
        ...
        if (request('grid') == 'share') :
            $data['stats'] = [
                ...
                'cookies' => [
                    'count' => Cache::remember("stats.cookies.count|$cachingPeriod|$brand|$country", $ttl, function () use ($period, $brand, $country) {
                        return current_project()->logs()->country($country)->brand($brand)->whereBetween('created_at', [$period['start'], $period['end']])->where('name', 'cookie_consent')->get()->count();
                    }),
                    'accepted' => Cache::remember("stats.cookies.accepted|$cachingPeriod|$brand|$country", $ttl, function () use ($period, $brand, $country) {
                        return current_project()->logs()->country($country)->brand($brand)->where('name', 'cookie_consent')->where('values->accepted', 'yes')->whereBetween('created_at', [$period['start'], $period['end']])->get()->count();
                    }),
                    'rejected' => Cache::remember("stats.cookies.rejected|$cachingPeriod|$brand|$country", $ttl, function () use ($period, $brand, $country) {
                        return current_project()->logs()->country($country)->brand($brand)->where('name', 'cookie_consent')->where('values->accepted', 'no')->whereBetween('created_at', [$period['start'], $period['end']])->get()->count();
                    }),
                ]
            ];
            return $data;
