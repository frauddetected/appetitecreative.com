...
class PewPewController extends MainController
{
    public function index()
    {
        if(request('period')):
            $period = explode(".", request('period'));
            $startDate = Carbon::createFromTimestamp($period[0])->startOfDay();
            $endDate = Carbon::createFromTimestamp($period[1])->endOfDay();
        else:
        ...
        $data['stats'] = [
            ...
            'users' => [
                'count' => Cache::remember("stats.users.count|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->logs()->groupBy('sessid')->whereBetween('created_at', [$period['start'], $period['end']])->get()->count();
                })
            ],
            'participants' => [
                'count' => Cache::remember("stats.users.count|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->logs()->where('name','post_score')->whereBetween('created_at', [$period['start'], $period['end']])->groupBy('sessid')->get()->count();
                })
            ],
            'submissions' => [
            ...
            'scientist_view' => [
                'count' => Cache::remember("stats.scientist_view.count|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->logs()->whereBetween('created_at', [$period['start'], $period['end']])->where('name','scientist_view')->get()->count();
                }),
                'top' => Cache::remember("stats.scientist_view.top|$cachingPeriod", $ttl, function() use ($period){                
                    return current_project()->logs()->where('name','scientist_view')
                    ->groupBy('values->name')
                    ->orderBy('views','desc')
                    ->whereBetween('created_at', [$period['start'], $period['end']])
                    ->get(array(
            ...
            'scientist_select' => [
                'count' => Cache::remember("stats.scientist_select.count|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->logs()->where('name','scientist_select')->whereBetween('created_at', [$period['start'], $period['end']])->get()->count();
                }),
                'top' => Cache::remember("stats.scientist_select.top|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->logs()->where('name','scientist_select')
                        ->groupBy('values->name')
                        ->whereBetween('created_at', [$period['start'], $period['end']])
                        ->orderBy('views','desc')
                        ->get(array(
            ...
            'leaderboard' => [
                'count' => Cache::remember("stats.leaderboard.count|$cachingPeriod", $ttl, function() use ($period){
                    return current_project()->leaderboard()->whereBetween('created_at', [$period['start'], $period['end']])->groupBy('email')->get()->count();
                }),
                'top' => Cache::remember("stats.leaderboard.top|$cachingPeriod", $ttl, function() use ($period){
                            $lb = current_project()->leaderboard();
                            $lb = $lb->select('email', 'name', 'details', DB::raw('sum(score) as score'));
                            $lb = $lb->whereBetween('created_at', [$period['start'], $period['end']])->orderBy('score', 'desc')->groupBy('email')->limit(10)->get();
                            return $lb;
                        })
            ]
        ];
        ...
        return Inertia::render('Dashboard/Projects/PewPew/Index', $data);
    }
    ...
    public function exports()
    {
        if(request('period') != 'all'):
            ...
            $filename = 'pewpew_export_custom_period.xlsx';
        else:
            $startDate = Carbon::now()->subMonth(3);
            $endDate = Carbon::now();
            $filename = 'pewpew_export_all.xlsx';
        endif;
