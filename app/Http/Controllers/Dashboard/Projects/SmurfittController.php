...
class SmurfittController extends MainController
{
    public function index()
    {
        ...
        */
        ...
        if(request('grid') == 'hearts'):
            
            $data['stats'] = [
                'hearts' => [
                    'count' => Cache::remember("stats.click_heart.count|$cachingPeriod|$brand", $ttl, function() use ($period, $brand){
                        return current_project()->logs()->brand($brand)->where('name','click_heart')->whereBetween('created_at', [$period['start'], $period['end']])->get()->count();
                    }),
                    'top' => Cache::remember("stats.click_heart.top|$cachingPeriod|$brand", $ttl, function() use ($period, $brand){
                        return current_project()->logs()->brand($brand)->where('name','click_heart')
                            ->groupBy('values->section')
                            ->whereBetween('created_at', [$period['start'], $period['end']])
                            ->orderBy('views','desc')
                            ->get(array(
                                'values->section as section',
                                DB::raw('COUNT(*) as "views"')
                            ));
                    }),
                ]
        ...
        return Inertia::render('Dashboard/Projects/Smurfitt/Index', $data);
    }

    public function exportHearts()
    {
        $startDate = Carbon::now()->subYear(1);
        $endDate = Carbon::now();
        $filename = "follow_ups_all.xlsx";

        function emailsGenerator($startDate, $endDate)
        {
            $uniques = current_project()
                        ->logs()
                        ->where('name','=','click_heart')
        ...
        return (new FastExcel(emailsGenerator($startDate, $endDate)))->download($filename, function ($item) {
            ...
            $export['Section'] = isset($item->values->section) ? $item->values->section : null;
            $export['E-mail'] = isset($item->values->email) ? $item->values->email : null;
