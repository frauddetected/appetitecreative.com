...
class WoodlandsController extends MainController
{
    public function index()
    {
        ...
        */
        ...
        if(request('grid') == 'share'):
            $data['stats'] = [
                'share' => [
                    'count' => Cache::remember("stats.downloads.count|$cachingPeriod|$brand", $ttl, function() use ($period, $brand){
                        return current_project()->logs()->brand($brand)->whereBetween('created_at', [$period['start'], $period['end']])->where('name','downloads')->get()->count();
                    }),
                    'top' => Cache::remember("stats.downloads.top|$cachingPeriod|$brand", $ttl, function() use ($period, $brand){
                        return current_project()->logs()->where('name','downloads')
                                ->brand($brand)
                                ->groupBy('values->page_title')
                                ->orderBy('views','desc')
                                ->whereBetween('created_at', [$period['start'], $period['end']])
                                ->get(array(
                                    'values->page_title as method',
                                    DB::raw('COUNT(*) as "views"')
                                ));
                    }),
                ]
        ...
        return Inertia::render('Dashboard/Projects/Woodlands/Index', $data);
    }
