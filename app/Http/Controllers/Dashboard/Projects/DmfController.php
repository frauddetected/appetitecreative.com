...
class DmfController extends MainController
{
    public function index()
    {
        ...
        return Inertia::render('Dashboard/Projects/Dmf/Index', $data);
    }
    ...
    public function prizes()
    {
        ...
        if(request('export') == true):
            ...
            return (new FastExcel( $leaderboard ))->download('export.xls', function ($item) {
                return [
                    'Name' => $item->name,
                    'E-mail' => $item->email,
                    'Address' => isset($item->user->profile->address) ? ($item->user->profile->address.PHP_EOL.$item->user->profile->city.PHP_EOL.$item->user->profile->postcode) : null,
        ...
        return Inertia::render('Dashboard/Projects/Dmf/Prizes', $data);
    }
    ...
    public function participantUpdate()
    {
        ...
        $json = $participant->profile;
        $json->address = request('address');
        $json->city = request('city');
        $json->postcode = request('postcode');
