<template>
    <app-layout title="Dashboard">
        ...
        <template #header>
            ...
            <nav class="flex items-center"> 
                ...
                <button class="hover:bg-ms-gray-20 text-ms-gray-160 p-3 flex items-center is-dropdown">
                    <i class="ms-Icon ms-Icon--ProjectCollection mr-2"></i> Filter by <strong class="ml-3">{{ brand == 'All' ? brand : brand.title }}</strong>
                    <ul>
                ...
                <div v-if="grid.scans" class="w-7/12 p-6 bg-white rounded-sm mr-3">
                  ...
                  <div class="flex w-full mb-8 pl-8 pr-2">
                      ...
                      <div class="w-4/12 text-center" v-tippy="{ content: 'Average number of scans per user.' }">
                          ...
                          <h1 class="text-4xl ml-auto font-bold">
                              {{ nFormatter(stats.scans.count / stats.registrations.count, 1) }}
                          </h1>
                      </div>
                      <div class="w-4/12 text-center" v-tippy="{ content: 'Average number of scans per user.' }">
        ...
        <div class="flex mt-6">
            ...
            <!-- Selfies -->
            <transition name="fade" mode="out-in">
            <div v-if="grid.selfies" class="w-1/3 bg-white rounded-sm mr-3">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <h2 class="flex w-full border-b p-4 mb-4">
                        Selfies by  
                        <div class="filters-dropdown">
                            <span>{{ filters.selfies }}</span>
                            <aside>
                                <ul>
                                    <li :class="{ 'font-bold' : filters.selfies == 'Views' }" @click="filters.selfies='Views'">Views</li>
                                    <li :class="{ 'font-bold' : filters.selfies == 'Taken' }" @click="filters.selfies='Taken'">Taken</li>
                                </ul>
                            </aside>
                        </div>
                    </h2>
                    ...
                    <div class="flex w-full justify-center mt-4">
                        <div class="w-1/3 text-center" v-tippy="{ content: `<div class='p-2 w-40 text-center'>Total number of selfie filters viewed<div>`, followCursor: false }">
                            <h2 class="uppercase text-xs">Filter Views</h2>
                            <h1 class="text-2xl ml-auto font-bold">
                                {{ nFormatter(stats.selfie_select.count, 1) }}
                            </h1>
                        </div>
                        <div class="w-1/3 text-center" v-tippy="{ content: `<div class='p-2 flex flex-col w-40 text-center'>Total number of selfies taken <br> <strong class='pt-2'>${calcPerc(stats.selfie_take.count,stats.selfie_select.count)}</strong><div>`, followCursor: false }">
                            <h2 class="uppercase text-xs">Selfies Taken</h2>
                            <h1 class="text-2xl ml-auto font-bold">
                                {{ nFormatter(stats.selfie_take.count, 1) }}
                            </h1>
                        </div>
                    </div>
                </div>
                ...
                <section class="flex-table p-6 mt-8">
                    <div class="flex-table-header">
                        ...
                        <div class="col text-right">{{ filters.selfies }}</div>
                    </div>
                    <div
                        v-tippy="{ content: `<div class='p-2'>${top.filter} <strong class='ml-2'>${top.views} (${calcPerc(top.views, stats[filters.selfies=='Views'?'selfie_select':'selfie_take'].count)})</strong></div>`, followCursor: true }"
                        v-for="top in stats[filters.selfies=='Views'?'selfie_select':'selfie_take'].top"
                        class="flex-table-row">
                            <div class="col font-bold">{{ unCamelCase(top.filter) }}</div>
                            <div class="col text-right">{{ top.views }}</div>
                            <div :style="{ width: calcPerc(top.views, stats[filters.selfies=='Views'?'selfie_select':'selfie_take'].count) }" class="duration-300 left-0 bottom-0 border-b-2 absolute border-ms-orange-10"></div>
                    </div>
                </section>
            ...
            <!-- Games -->
            <transition name="fade" mode="out-in">
            <div v-if="grid.games" class="w-1/3 bg-white rounded-sm mx-3">
            ...
            <transition name="fade" mode="out-in">
            <div v-if="grid.leaderboard" class="w-1/3 bg-white rounded-sm ml-3">

                <h2 class="flex w-full p-4 border-b mb-4">
                    Leaderboard by 
                    <div class="filters-dropdown">
                        <span>{{ filters.registrations }}</span>
                        <aside>
                            <ul>
                                <li :class="{ 'font-bold' : filters.registrations == 'Users' }" @click="filters.registrations='Users'">Users</li>
                                <li :class="{ 'font-bold' : filters.registrations == 'Score' }" @click="filters.registrations='Score'">Score</li>
                            </ul>
                        </aside>
                    </div>
                    <nav class="ml-auto flex">
                        ...
                        <VDropdown>
                            <button class="hover:text-ms-magenta-c239b3">
                                <i class="ms-Icon ms-Icon--Calendar mr-3"></i>
                            </button>
                            <template #popper>

                                <p>Leaderboard Period</p>

                                <select v-if="project.id == 3" v-model="leaderboardPeriod" name="" id="">
                                    <option value="jun-jul">Jun 8 - Jul 27</option>
                                    <option value="apr-jun">Apr 18 - Jun 8</option>
                                    <option value="mar-apr">Mar 4 - Apr 18</option>
                                    <option value="jan-mar">Jan 17 - Mar 4</option>    
                                </select>

                                <select v-else v-model="leaderboardPeriod" name="" id="">
                                    <option value="jul-jul">Jul 5 - Jul 31</option>
                                    <option value="may-jul">May 20 - Jul 4</option>
                                    <option value="apr-may">Apr 15 - May 20</option>
                                    <option value="mar-apr">Mar 1 - Apr 15</option>
                                    <option value="jan-feb">Jan 13 - Feb 28</option> 
                                </select>

                            </template>
                        </VDropdown>

                        <button class="hover:text-ms-magenta-30" @click="filterUserLeaderboad.panel=true">
                            <i class="ms-Icon ms-Icon--Search mr-3"></i>
                        </button>

                        <a class="hover:text-green-600" :href="`/export/leaderboard`">
                            <i class="ms-Icon ms-Icon--ExcelDocument mr-2"></i>
                        </a>
                ...
                <div v-if="filters.registrations == 'Users'">
                    ...
                    <section class="flex-table p-6 mt-8">
                        ...
                        <div
                            v-for="top in stats.leaderboard" 
                            class="flex-table-row">
                                <div class="col w-1/3 font-bold">
                                    <span class="cursor-pointer" @click="filterUserLeaderboad.panel=true;filterUserLeaderboad.input=top.email;filterUserLeaderboadSubmit();">
                                        {{  top.name.length > 12 ? top.name.substring(0,12) + '..' : top.name }}
                                    </span>
                                </div>
                                <div class="col w-1/3 text-xs">{{ top.email }}</div>
                                <div class="col w-1/3 text-right">{{ top.score }}</div>
                        </div>
                    </section>
                </div>

                <div v-if="filters.registrations == 'Score'" class="">

                    <p class="text-xs mx-4 p-4 bg-green-100 text-center">Leaderboard data between {{ moment(between.start).format('ll') }} and {{ moment(between.end).format('ll') }}</p>
                    
                    <div class="px-4 mt-8">
                        <column-chart height="240px" :colors="['#c239b3']" :data="charts.leaderboard"></column-chart>
                    </div>
         ...
         <div class="flex mt-6">
            ...
            <div v-if="grid.share" class="w-1/3 bg-white rounded-sm mr-3">
            ...
            <div v-if="grid.profile" class="w-1/3 bg-white rounded-sm mx-3">
                    ...
                    <div class="flex w-full mt-4 justify-center">
                        <div class="w-2/3 text-center">
                            <h2 class="uppercase text-xs">Avg Games Played <br> per Day</h2>
                            <h1 class="text-3xl ml-auto font-bold">
                                {{ nFormatter(stats.users.avg_plays.gamesDays, 2) }}
                            </h1>
                        </div>
                    </div>
                    ...
                    <div class="flex w-full mt-8 justify-center">
                        <div class="w-2/3 text-center">
                            <h2 class="uppercase text-xs">Avg Games Played <br> per User</h2>
                            <h1 class="text-3xl ml-auto font-bold">
                                {{ nFormatter(stats.users.avg_plays.gamesParticipants, 2) }}
                            </h1>
                        </div>
                    </div>
                    ...
                    <!--
                    <column-chart :colors="['#00bcf2','#8378de','#c239b3','#0078d4','#005b70']" :library="{ animation: { duration: 3000 }}" height="300px" class="mt-5" :data="[['1 Play', 1], ['2 Plays', 0], ['3 Plays', 0], ['4 Plays', 0], ['5 Plays', 0]]"></column-chart>
                    -->
...
<script>
    ...
    export default {
        ...
        {            
            ...
            axios.post('/', { grid: 'scans', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'profile', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'leaderboard', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
                    this.stats.leaderboard = r.data.stats.leaderboard
                    this.charts.leaderboard = r.data.charts.leaderboard
                    this.grid.leaderboard = 1         
                })
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'games', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'selfie', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'share', period: this.fullUnixRange, filter: { brand: this.brand.id } }).then(r => {
