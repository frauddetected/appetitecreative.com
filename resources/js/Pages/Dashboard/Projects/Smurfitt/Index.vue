<template>
    <app-layout title="Dashboard">
        ...
        <div class="flex gap-x-6 mt-6">
            ...
            <!-- Selfies -->
            <transition name="fade" mode="out-in">
            <div v-if="grid.hearts" class="w-1/3 bg-white rounded-sm">

                <div class="w-full flex flex-col">
                    <h2 class="flex w-full justify-between border-b p-4 mb-4">
                        Likes (Follow Ups) 
                        <a class="hover:text-green-600" :href="`/export/hearts`">
                ...
                <section class="flex-table p-6 mt-8">
                    <div class="flex-table-header">
                        ...
                        <div class="col text-right">Counting</div>
                    </div>
                    <div
                        v-for="top in stats.hearts.top"
                        class="flex-table-row">
                            <div class="col font-bold">{{ top.section }}</div>
                            <div class="col text-right">{{ top.views }}</div>
                            <div :style="{ width: calcPerc(top.views, stats.hearts.count) }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-orange-10/10"></div>
                    </div>
                </section>
...
<script>
    ...
    export default {
        ...
        {            
            ...
            setTimeout(() => {
                axios.post('/', { grid: 'hearts', period: this.fullUnixRange, filter: { brand: this.brand.id, country: this.country } }).then(r => {
                    this.stats.hearts = r.data.stats.hearts
                    this.grid.hearts = 1         
                })
            }, 4000);
