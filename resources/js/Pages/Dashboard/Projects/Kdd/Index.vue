<template>
    <app-layout title="Dashboard">
                        ...
                        <div class="flex flex-col">
                            <form action="" class="mb-5 w-40">
                                ...
                                <label class="flex items-center my-1">
                                    <input type="checkbox" v-model="exporting.fields" value="school" class="mr-1"> School
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" v-model="exporting.fields" value="origin" class="mr-1"> Origin Game
                                </label>
                                <label class="flex items-center my-1">
                                    <input type="checkbox" v-model="exporting.fields" value="package" class="mr-1"> Package
                                </label>
                                <label class="flex items-center pl-5">
                                    <input type="checkbox" v-model="exporting.fields" value="country" class="mr-1"> Country
                                </label>
                                <label class="flex items-center pl-5">
                                    <input type="checkbox" v-model="exporting.fields" value="language" class="mr-1"> Language
                                </label>
                                <label class="flex items-center my-1">
                            ...
                            <button @click="exportDownload" class="border ml-auto border-ms-gray-160 px-4 py-1 hover:bg-ms-gray-40 ml-2 font-bold">Download</button>
        ...
        <div class="flex mt-6">
            ...
            <!-- Selfies -->
            <div class="w-1/3 bg-white rounded-sm mr-3">
            ...
            <!-- Games -->
            <div class="w-1/3 bg-white rounded-sm mx-3">
            ...
            <div class="w-1/3 bg-white rounded-sm ml-3">
                ...
                <div v-if="filters.registrations == 'Origin'">

                    <div class="flex w-full mt-8 justify-center">
                        <div class="w-2/3 text-center" v-tippy="{ content: `<div class='p-2 w-40 text-center'>Total number of registrations<div>`, followCursor: false }">
                            <h2 class="uppercase text-xs">Total Submissions</h2>
                            <h1 class="text-2xl ml-auto font-bold">
                                {{ nFormatter(stats.participants.count, 1) }}
                            </h1>
                        </div>
                    ...
                    <section class="flex-table p-6 mt-8">
                        ...
                        <div
                            v-tippy="{ content: `<div class='p-2'>${top.origin_value} (${top.origin}) <strong class='ml-2'>${top.views} (${calcPerc(top.views, stats.participants.count)})</strong></div>`, followCursor: true }"
                            v-for="top in stats.participants.origin" 
                            class="flex-table-row">
                                <div class="col font-bold">{{ top.origin_value }}</div>
                                <div class="col text-right">{{ top.views }}</div>
                                <div :style="{ width: calcPerc(top.views, stats.participants.count) }" class="duration-300 left-0 bottom-0 border-b-2 absolute border-ms-magenta-10"></div>
                        </div>
                    </section>
                </div>

                <div v-if="filters.registrations == 'Score'" class="p-6">
...
<script>
    ...
    export default {
        ...
        computed: {
            rangeUnixStart(){
                return parseInt((new Date(this.range.start).getTime() / 1000).toFixed(0))
            },
            rangeUnixEnd(){
                return parseInt((new Date(this.range.end).getTime() / 1000).toFixed(0))
            }
        },
        methods: {
            exportDownload()
