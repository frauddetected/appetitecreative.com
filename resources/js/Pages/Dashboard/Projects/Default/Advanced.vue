<template>
    <app-layout title="Dashboard">
        ...
        <div class="flex gap-x-6 mt-6">
            ...
            <div v-if="grid.games" class="w-1/3 bg-white rounded-sm">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <div
                        v-tippy="{ content: `<div class='p-2'>origin <strong class='ml-2'>${origin.views} (${calcPerc(origin.views, stats.leaderboard_stats.count)})</strong></div>`, followCursor: true }"
                        v-for="(origin) in stats.leaderboard_stats.origin" 
                        class="flex-table-row">
                            <div class="col font-bold capitalize">{{ origin.origin_value }}</div>
                            <div class="col text-right">{{ origin.views }}</div>
                            <div :style="{ width: calcPerc(origin.views, stats.leaderboard_stats.count) }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-orange-20/10"></div>
                    </div>
                </div>
                </div>
            ...
            <div v-if="grid.share" class="w-1/3 bg-white rounded-sm">
                ...
                <section class="flex-table p-6 mt-8">
                    ...
                    <div class="flex-table-row">
                        <div class="col font-bold capitalize">WhatsApp</div>
                        <div class="col text-right">0</div>
                        <div class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10" style="width: 0%;"></div>
                    </div>
                    <div class="flex-table-row">
                        <div class="col font-bold capitalize">Pinterest</div>
                        <div class="col text-right">0</div>
                        <div class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10" style="width: 0%;"></div>
                    </div>
                    <div class="flex-table-row">
                        <div class="col font-bold capitalize">Facebook</div>
                        <div class="col text-right">0</div>
                        <div class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10" style="width: 0%;"></div>
                    </div>
                    <div class="flex-table-row">
                        <div class="col font-bold capitalize">Twitter</div>
                        <div class="col text-right">0</div>
                        <div class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10" style="width: 0%;"></div>
                    </div>
                    <div class="flex-table-row">
                        <div class="col font-bold capitalize">Telegram</div>
                        <div class="col text-right">0</div>
                        <div class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10" style="width: 0%;"></div>
                    </div>
                    -->
            ...
            <div v-if="grid.games" class="w-1/3 bg-white rounded-sm">
                ...
                <div
                    ...
                    class="flex-table-row">
                        ...
                        <div class="col text-right">{{ top.views }}</div>
                        <div :style="{ width: calcPerc(top.views, stats.game_start.count) }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10"></div>
                </div>
                </section>
                ...
                <div
                    ...
                    class="flex-table-row">
                        <div class="col font-bold">{{ top.name }}</div>
                        <div class="col text-right">{{ top.views }}</div>
                        <div :style="{ width: calcPerc(top.views, stats.game_end.count) }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10"></div>
                </div>
                </section>
                ...
                <div
                    ...
                    class="flex-table-row">
                        ...
                        <div :style="{ width: parseInt((stats.game_end.top.filter(r => r.name == top.name)[0].views) * 100 / top.views) + '%' }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-magenta-10/10"></div>
                </div>
                </section>
        ...
        <div class="flex gap-x-6 mt-6">
            ...
            <!-- Selfies -->
            <transition name="fade" mode="out-in">
            <div v-if="grid.selfies" class="w-1/3 bg-white rounded-sm">
                ...
                <section class="flex-table p-6 mt-8">
                    ...
                    <div
                        ...
                        class="flex-table-row">
                            <div class="col font-bold">{{ unCamelCase(top.filter).replace('.deepar','') }}</div>
                            <div class="col text-right">{{ top.views }}</div>
                            <div :style="{ width: calcPerc(top.views, stats[filters.selfies=='Views'?'selfie_select':'selfie_take'].count) }" class="duration-300 left-0 bottom-0 h-full absolute bg-ms-orange-10/10"></div>
                    </div>
                </section>
            
            </div>
            <div v-else class="w-1/3 bg-white rounded-sm flex justify-center items-center">
                <Loader type="table" />
            </div>
            </transition>
                    ...
                    <nav class="ml-auto flex">
                        ...
                        <VDropdown>
                            ...
                            <template #popper>
                                ...
                                <select v-model="leaderboardPeriod" name="" id="">
    ...
    export default {
        ...
        {            
            ...
            axios.post('/', { grid: 'scans', period: this.fullUnixRange, filter: { brand: this.brand.id, country: this.country } }).then(r => {
                ...
                this.stats.leaderboard_stats = r.data.stats.leaderboard
